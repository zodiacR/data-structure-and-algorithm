...
void Dijkstra(Graph* g, int src){
    ...
    while(!IsEmpty(heap)){
        ...
        while(n){
            if (IsInMinHeap(heap, n->dest) && distance[u->v] != INT_MAX &&
                    distance[u->v] + n->weight < distance[n->dest]){
                distance[n->dest] = distance[u->v] + n->weight;
                Decrease(heap, n->dest, distance[n->dest]);
            }
            n = n->next;
        }
    }

    printArr(distance, g->V);
}
